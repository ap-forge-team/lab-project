        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border">
              ...
              <th className="text-left py-2.5 text-muted-foreground text-[11px] font-semibold uppercase tracking-wider hidden lg:table-cell">Date</th>
            </tr>
          </thead>
          <tbody>
            {(data || []).map((booking) => (
            ...
            {(!data || data.length === 0) && (
              <tr>
                <td colSpan={5} className="py-6 sm:py-8 text-center text-muted-foreground text-xs">No recent bookings</td>
              </tr>
            )}
          </tbody>
        </table>
