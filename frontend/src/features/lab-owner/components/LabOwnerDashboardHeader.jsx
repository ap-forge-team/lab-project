...
const LabOwnerDashboardHeader = ({ dateRange, onDateRangeChange }) => {
  ...
  const dateRangeText = useMemo(() => {
    ...
    if (dateRange?.to) {
      return `Until ${format(dateRange.to, 'dd MMM yyyy')}`
    }
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-foreground">
            Welcome back, Lab Owner 👋
          </h1>
          <p className="text-muted-foreground text-xs sm:text-sm mt-1">
            Here's an overview of your lab performance and operations.
          </p>
        </div>
...
export default LabOwnerDashboardHeader
