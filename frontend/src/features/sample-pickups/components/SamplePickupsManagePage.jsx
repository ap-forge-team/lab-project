    ...
    <section className="mx-auto max-w-[1500px] space-y-4 lg:space-y-5">
      ...
      <div className="hidden sm:flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <SearchInput value={search} onChange={(e) => { setSearch(e.target.value); setPage(1) }} placeholder="Search sample pickups..." />
          <FilterButton
            onClick={(e) => {
      ...
      <div className="flex sm:hidden items-center gap-2">
        <SearchInput value={search} onChange={(e) => { setSearch(e.target.value); setPage(1) }} placeholder="Search sample pickups..." className="flex-1" width="w-full" />
        <FilterButton
          onClick={(e) => {
            const rect = e.currentTarget.getBoundingClientRect()
            setFilterPanelOpen({ top: rect.bottom + 8, left: Math.max(16, rect.right - 680) })
